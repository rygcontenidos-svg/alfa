...
export default function UnirFlechas({
  ej,
  moduloId,
}: {
  ej: UnirFlechasType;
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            {ej.izquierda.map((it) => (
              <div
                key={it.id}
                className="rounded-lg border border-borde bg-gray-50 px-3 py-2 text-sm text-grafito"
              >
                {it.texto}
              </div>
            ))}
          ...
          <div className="space-y-2">
            {ej.derecha.map((it) => (
              ...
              >
                {mostrar && (
                  <span className="text-verde font-medium mr-1">⇢ </span>
                )}
                {it.texto}
              </div>
            ))}
          ...
          <div className="mt-3 rounded-lg bg-verde-claro border border-verde px-3 py-2">
            ...
            <div className="space-y-0.5">
              {ej.pares.map((p) => {
                ...
                return (
                  <p key={p.izquierda_id} className="text-xs text-grafito">
                    {izq?.texto} ↔ {der?.texto}
                  </p>
                );
              })}
