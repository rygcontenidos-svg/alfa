      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead><tr>{ej.columnas.map((col, i) => <th key={i} className="border border-borde bg-gray-50 px-3 py-2 text-left font-semibold text-grafito" style={i === 0 ? { width: "55%" } : { minWidth: "200px" }}>{col}</th>)}</tr></thead>
            <tbody>
              {ej.filas.map((fila) => (
                <tr key={fila.id}>
